...
ScreenCornmm = np.array([[0.0,0.0],[-2.8,616.5],[746.6,0.0],[745.8,615.8]],np.float32)

ScreenCornpx = np.array([[0,0],[50,800],[1200,200],[1500,1200]],np.int32)

retval, mask = cv2.findHomography(ScreenCornpx, ScreenCornmm, cv2.RANSAC,5.0)

#print(retval)

#QR Code Coordinates in mm in relation to screen UL, LL, UR, LR
QRCornmm = np.array([[10.7,7.7],[6.9,536.6],[675.2,9.5],[675.9,541.3],
...
for barcode in decode(img):
    ...
    #print(pts.shape)
    #print(QRCornpx.shape)

    if myData.find('upper-left') > -1:
        x = pts[0][0][0]
        y = pts[0][0][1]

        QRCornpx[0] = pts[0][0] #UL corner of UL QR Code
        QRCornpx[1] = pts[0][1] #LL corner of UL QR Code
        QRCornpx[2] = pts[0][2] #UR corner of UL QR Code
        QRCornpx[3] = pts[0][3] #LR corner of UL QR Code
    else:
        print("UL QR Code not found!")
        
    if myData.find('Lower-left') > -1:
        QRCornpx[4] = pts[0][0] #UL corner of LL QR Code
        QRCornpx[5] = pts[0][1] #LL corner of LL QR Code
        QRCornpx[6] = pts[0][2] #UR corner of LL QR Code
        QRCornpx[7] = pts[0][3] #LR corner of LL QR Code
    else:
        print("LL QR Code not found!")
    
    if myData.find('Upper-right') > -1:
        QRCornpx[8] = pts[0][0] #UL corner of UR QR Code
        QRCornpx[9] = pts[0][1] #LL corner of UR QR Code
        QRCornpx[10] = pts[0][2] #UR corner of UR QR Code
        QRCornpx[11] = pts[0][3] #LR corner of UR QR Code
    else:
        print("UR QR Code not found!")
        
    if myData.find('Lower-right') > -1:
        QRCornpx[12] = pts[0][0] #UL corner of LR QR Code
        QRCornpx[13] = pts[0][1] #LL corner of LR QR Code
        QRCornpx[14] = pts[0][2] #UR corner of LR QR Code
        QRCornpx[15] = pts[0][3] #LR corner of LR QR Code
    else:
        print("LR QR Code not found!")
    ...
    #We can find the actual screen corners using this. Not done with this.
    #(ul, ll, ur, lr) = barcode.polygon
    
    # get coordinates (x,y)
    #arr = np.array([c.x, c.y])
    #print("Center of QR Code:")
    #print(arr)
    ...
    #print(myData)
    #print("QR Coordinates in Pixels (Upper left, upper right, lower right, and lower left):")    
    print(pts)
    #print("\n")
    ...
    cv2.putText(img, myData,(pts2[0][0],pts2[0][1]),cv2.FONT_HERSHEY_SIMPLEX, 0.9,(0,0,255),3)
...
retval, mask = cv2.findHomography(QRCornpx, QRCornmm, cv2.RANSAC,5.0)
print(QRCornpx)
print(retval)
